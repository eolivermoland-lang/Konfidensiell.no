        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center'>
              ...
              <div className="space-y-4 mb-10 inline-block text-left" role="list">
                {[t.home_intro.point1, t.home_intro.point2, t.home_intro.point3].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 text-gray-200 font-bold text-sm sm:text-base" role="listitem">
                    <CheckCircle className="text-blue-500 flex-shrink-0" size={20} aria-hidden="true" /> {item}
                  </div>
                ))}
              </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-6 w-full'>
              <article className='glass-card p-8 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] flex flex-col justify-between h-full border border-white/5 hover:border-blue-500/30 transition-colors'>
                <div>
                  <Target className='mb-6 text-blue-500' size={40} aria-hidden="true" />
                  <h3 className='text-white font-black text-lg sm:text-xl mb-4 uppercase tracking-tighter'>{t.home_intro.card1_title}</h3>
                  <p className='text-gray-400 text-xs sm:text-sm leading-relaxed'>{t.home_intro.card1_desc}</p>
                </div>
              </article>
              <article className='glass-card p-8 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] flex flex-col justify-between h-full border border-white/5 hover:border-purple-500/30 transition-colors'>
                <div>
                  <UserCheck className='mb-6 text-purple-500' size={40} aria-hidden="true" />
                  <h3 className='text-white font-black text-lg sm:text-xl mb-4 uppercase tracking-tighter'>{t.home_intro.card2_title}</h3>
                  <p className='text-gray-400 text-xs sm:text-sm leading-relaxed'>{t.home_intro.card2_desc}</p>
                </div>
              </article>
            </div>
          </div>
      ...
      <section className='py-24 sm:py-32 border-t border-white/5 glass-panel' aria-labelledby="process-heading">
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='text-center mb-16 sm:mb-24'>
            ...
            <p className="text-gray-400 text-lg sm:text-xl max-w-2xl mx-auto">{t.process.subheading}</p>
          </div>
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-12'>
            {processSteps.map((step, index) => (
              <article key={index} className='text-center group'>
                <div className='w-20 h-20 sm:w-24 sm:h-24 bg-blue-600/10 border border-blue-600/20 rounded-3xl flex items-center justify-center mx-auto mb-6 sm:mb-8 text-blue-500 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500'>
